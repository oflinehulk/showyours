-- Allow rpc_host_add_squad to work when tournament status is 'ongoing' (for team swap feature)
-- Also exclude withdrawn registrations from duplicate check
...
BEGIN
  ...
  -- Allow both registration_closed AND ongoing (for swap during group stage)
  IF v_tournament.status NOT IN ('registration_closed', 'ongoing') THEN
    RAISE EXCEPTION 'Squads can only be added when registration is closed or tournament is ongoing';
  END IF;

  -- Only enforce max_squads during registration_closed (swap is a replacement, not addition)
  ...
  -- Exclude withdrawn registrations from duplicate check
  IF EXISTS (
