-- Fix roster snapshot system:
-- 1. Fix rpc_host_add_squad: member_status was 'substitute' for pos>5 (violates CHECK, should be 'active')
-- 2. Unlock rosters when tournament reopens registration
-- 3. Add RPC for host to manually recapture roster snapshots

-- 1. Fix rpc_host_add_squad — all members should be 'active' regardless of position
...
-- 2. Update lock_tournament_rosters to also UNLOCK rosters when reopening registration
