-- Fix search_profiles function to handle contacts stored as JSON string
...
BEGIN
  ...
  SELECT 
    ...
    p.rank,
    p.main_role,
    -- Parse contacts properly (handle both string and JSONB)
    CASE 
  ...
  WHERE 
    ...
    -- Search by IGN, MLBB ID, or WhatsApp (in contacts JSON string)
    AND (
      LOWER(p.ign) ILIKE '%' || search_lower || '%'
      OR p.mlbb_id ILIKE '%' || search_term || '%'
      OR p.contacts::text ILIKE '%' || search_term || '%'
    )
