...
CREATE OR REPLACE FUNCTION public.rpc_host_edit_roster(
  ...
  p_new_ign text DEFAULT NULL,
  p_new_mlbb_id text DEFAULT NULL,
  p_new_role text DEFAULT 'substitute',
...
BEGIN
  ...
  SET roster_snapshot = (
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        'id', tsm.id, 'ign', tsm.ign, 'mlbb_id', tsm.mlbb_id,
        'role', tsm.role, 'position', tsm.position
      )
    ), '[]'::jsonb)
  ...
  -- Audit log (use v_old_ign instead of v_old_member.ign to avoid unassigned record error)
