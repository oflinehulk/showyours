-- Add booked_slots to scheduling context so public page shows already-scheduled slots
...
BEGIN
  ...
  -- All booked (scheduled) slots in this tournament
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'date', to_char(m.scheduled_time AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD'),
      'time', to_char(m.scheduled_time AT TIME ZONE 'Asia/Kolkata', 'HH24:MI')
    )
  ), '[]'::jsonb)
  INTO v_booked_slots
  FROM tournament_matches m
  ...
  RETURN jsonb_build_object(
    ...
    'squad_logo', v_rec.squad_logo, 'matches', v_matches,
    'submitted_at', v_submitted_at, 'booked_slots', v_booked_slots
  );
END;
$$;
