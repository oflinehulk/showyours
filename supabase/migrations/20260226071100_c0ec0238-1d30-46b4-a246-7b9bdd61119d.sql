...
-- 1. Fix check_duplicate_player_in_tournament(): guard normalize + skip check for empty mlbb_id
...
BEGIN
  -- Only normalize when mlbb_id has actual content
  ...
  -- Skip duplicate check entirely for empty mlbb_id
  IF NEW.mlbb_id IS NULL OR trim(NEW.mlbb_id) = '' THEN
    RETURN NEW;
  END IF;
...
CREATE OR REPLACE FUNCTION public.rpc_host_add_squad(p_tournament_id uuid, p_squad_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 3. Drop and recreate the unique index with mlbb_id <> '' guard
DROP INDEX IF EXISTS unique_active_player_per_tournament;
CREATE UNIQUE INDEX unique_active_player_per_tournament
  ON public.tournament_squad_members (tournament_squad_id, mlbb_id)
